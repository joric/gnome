...
public class FlyCamera : MonoBehaviour {
	...
	void Update () {
		...
		if (!rotateOnlyIfMousedown || (rotateOnlyIfMousedown && Input.GetMouseButton(mouseButton))) {
		...
		p = p * Time.deltaTime * 0.05f;
		Vector3 newPosition = transform.position;
		if (Input.GetKey(KeyCode.Space) || (movementStaysFlat && !(rotateOnlyIfMousedown && Input.GetMouseButton(mouseButton)))){ //If player wants to move on X and Z axis only
